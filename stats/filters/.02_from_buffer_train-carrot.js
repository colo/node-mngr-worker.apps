...
module.exports = function(payload){
  ...
  let filter = function(doc, opts, next, pipeline){
    ...
    //   Array.each(buffer, function(doc){
        ...
        if(doc && doc.id === 'periodical' && doc.metadata && doc.metadata.from === table && doc.data){
          Array.each(doc.data, function(doc_data){
            // debug('doc_data', doc_data)
            Array.each(doc_data, function(real_data){
              // debug('real_data', real_data)
        ...
        Object.each(host_data, function(real_data, path){
          ...
          //   first = doc_data[doc_data.length - 1].metadata.timestamp;
            ...
            Array.each(real_data, function(group, group_index){
              ...
              last = (group.metadata.timestamp > last) ? group.metadata.timestamp : last
