...
let debug = require('debug')('Server:Apps:Educativa:Purge:All:Filters:Filters:from_default_query_delete_until_last_hour');
let debug_internals = require('debug')('Server:Apps:Educativa:Purge:All:Filters:Filters:from_default_query_delete_until_last_hour:Internals');
...
module.exports = function(payload){
  ...
  let filter = function(doc, opts, next, pipeline){
    ...
    if(doc && doc.id === 'default' && doc.data && doc.metadata && doc.metadata.from === table){
      ...
      let end = roundSeconds(Date.now() - 5 * MINUTE)
      ...
      Array.each(doc.data, function(group, index){
        ...
        Array.each(group.hosts, function(host){
          pipeline.get_input_by_id('input.all').fireEvent('onRange', {
            id: "range",
            Range: "posix 0-"+end+"/*",
            query: {
              "q": [
