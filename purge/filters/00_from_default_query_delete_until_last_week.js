...
let debug = require('debug')('Server:Apps:Purge:Periodical:Filters:from_default_query_delete_until_last_week');
let debug_internals = require('debug')('Server:Apps:Purge:Periodical:Filters:from_default_query_delete_until_last_week:Internals');
...
module.exports = function(payload){
  ...
  let filter = function(doc, opts, next, pipeline){
    ...
    if(doc && doc.id === 'periodical' && doc.data && doc.metadata && doc.metadata.from === table){
      ...
      let end = roundSeconds(Date.now() - WEEK)
